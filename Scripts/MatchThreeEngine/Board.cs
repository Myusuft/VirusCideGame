...
namespace MatchThreeEngine
{
	public sealed class Board : MonoBehaviour
	{
		...
		[SerializeField] private TileTypeAsset[] valuePoints;
		...
		public event Action<TileTypeAsset, int> Score;
		...
		private void Start()
		{
			...
			Score += (type, count) => ScoreCounter.Instance.Score += (count * type.value);
		}
		...
		private async Task<bool> TryMatchAsync()
		{
			...
			while (match != null)
			{
				...
				Debug.Log("Tambah Poin");
				//ScoreCounter.Instance.Score +=  1 * match.Tiles.Length;
				...
				Score?.Invoke(Array.Find(tileTypes, tileType => tileType.id == match.TypeId), match.Tiles.Length);
